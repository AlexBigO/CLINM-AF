...
def build_tank(id: str):
    ...
    tank_tmpl.translation = [20 * CM, 20 * CM, 20 * CM]  # dummy position
...
def place_separator(base, separator, id: int) -> None:
    """
...
def build_rack(sim: gate.Simulation, pos_base_rack):
    ...
    for key, pos in mapping.items():
        tanks[key] = build_tank(key)
        tanks[key].translation = pos
        water_inside_tanks[key] = put_water_in_tank(sim, tanks[key], key)
        sim.add_volume(tanks[key])

    # borders
    border_left_tmpl, border_left = build_border(id="Left")
    border_right_tmpl, border_right = build_border(id="Right")
    ...
    # rods
    rod_front_tmpl, rod_front = build_rod("Front")
    rod_back_tmpl, rod_back = build_rod("Back")
